...
class RobotReader(object):
        ...
        for lineno, line in enumerate(Utf8Reader(file).readlines(), start=1):
            ...
            if line.lstrip().startswith('#'):
                if cells[0] == '':  # There is an initial empty cell, when #
                    cells.pop(0)
                #print(f"DEBUG: robotreader.read startswitch # cells={cells}")
                # populator.add(cells)
                # continue
    ...
    def sharp_strip(self, line):
        ...
        if line[:int(self._spaces)] == ' ' * self._spaces and "#" in line:
            chkrow = self._space_splitter.split(line)
            print(f"DEBUG: RFLib RobotReader sharp_strip returning row={chkrow[:]}")
            row = chkrow
        else:
